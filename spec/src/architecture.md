...
- <!-- req: system.role.coordinator; source: artifacts/d-inference/service_discovery/components.json#L30-L87 --> The coordinator MUST be the protocol authority for provider eligibility, routing decisions, trust-state checks, and accounting coordination.
- <!-- req: system.role.provider; source: artifacts/d-inference/service_discovery/components.json#L193-L315 --> A provider MUST be treated as independently operated infrastructure until enrollment and attestation establish its current eligibility state.
