...
| Kind | service |
| Language | go-module |
| Root Path | `analytics/cmd/analytics` |
| Manifest | `analytics/go.mod` |
| Internal Deps | analytics |
| External Apps | postgresql |

> Standalone read-only analytics service for network statistics and earnings leaderboards
...
- → **postgresql** (DB connection) — Read-only access to provider and earnings data
