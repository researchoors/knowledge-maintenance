...
| External Apps | coordinator-api, cloudflare-r2, apple-secure-enclave, apple-hypervisor, huggingface-hub, macos-launchd |

> EigenInference provider agent for Apple Silicon Macs with hardware-attested security
...
- → **coordinator** (WebSocket) — Provider registration, inference request handling, and attestation challenges
- ← **EigenInference** (subprocess) — Manages darkbloom binary lifecycle through subprocess spawning and monitoring
- → **EigenInferenceEnclave** (FFI call) — Secure Enclave attestation and challenge-response signing via FFI bridge
- → **cloudflare-r2** (API call) — Model and runtime package downloads
- → **apple-secure-enclave** (hardware call) — Hardware attestation blob generation and challenge signing
